/**
 * @module @kb-labs/analytics-cli/rest/utils/buffer-reader
 * Read events from WAL buffer
 */
...
/**
 * Read events from buffer directory
 */
export async function readEventsFromBuffer(
  bufferDir: string,
  ...
  try {
    ...
    // Read latest segments (limit to last 10 files for performance)
    const filesToRead = jsonlFiles.slice(0, 10);
 ...
 */
export async function countEventsFromBuffer(
  bufferDir: string,
  filters: EventFilters = {}
): Promise<number> {
  const events = await readEventsFromBuffer(bufferDir, { ...filters, limit: undefined, offset: undefined });
